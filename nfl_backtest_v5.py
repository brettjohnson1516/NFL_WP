...
def main() -> None:
    ...
    c = float(args.contracts)
    bets["contracts"] = c
    bets["staked"] = bets["cost_cents"] * c / 100.0
    bets["fee"] = kalshi_fee(np.full(len(bets), c), bets["cost_cents"].to_numpy(),
                             args.fee_coeff)
    ...
    if args.per_bet_out:
        cols = ["game_id", "play_id", "week", "month", "qtr", "wall", "entry", "ts",
                ...
                "edge", "state_wp_home", "state_score_diff_home",
