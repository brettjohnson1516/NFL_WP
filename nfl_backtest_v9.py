...
For every snap N in the 2025 regular season:
  ...
  * bet         = placed only when model_prob(side) - cost/100 >= --min-edge
                  ...
                  costs sum to 101c.
  * size        = edge-scaled, expanding, no look-ahead. Bets are walked in
                  time order and each one is z-scored against the edges of the
                  bets BEFORE it, size multiplier = 1 + slope*z clipped to
                  [--size-min, --size-max], normalised by the running average
                  multiplier so the average size tracks --contracts. Nothing
                  about the rest of the season is used. The first
                  --size-warmup bets are sized flat. Realised average size
                  lands near --contracts, not exactly on it.
...
def parse_args() -> argparse.Namespace:
    ...
    ap.add_argument("--size-warmup", type=int, default=100,
                    help="bets sized flat at the start, before there is a "
                         "distribution of edges to compare against")
...
def main() -> None:
    ...
    contracts, zscore = edge_scaled_size(
        bets["edge"].to_numpy(dtype=float),
        bets["ts"].to_numpy("datetime64[ns]"),
    ...
    print(f"\nsizing    expanding (no look-ahead), slope {args.size_slope:g}, "
          f"multiplier clipped to [{args.size_min:.2f}, {args.size_max:.2f}], "
          f"first {args.size_warmup} bets flat")
